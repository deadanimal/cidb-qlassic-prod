...
def docx_to_pdf_file(letter_template, context):
    response = docx_to_pdf_process_file(letter_template, context, True)
    return response

def docx_to_pdf_process_file(letter_template, context, download):
    template_path = letter_template.template_file.path
    fileName, fileExtension = os.path.splitext(letter_template.template_file.name)
